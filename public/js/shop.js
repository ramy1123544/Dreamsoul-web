// ── منطق المتجر مع D3.js ──// ── shop.js (نسخة مبسطة للاختبار) ──
...
async function loadShopItems() {
  ...
  console.log('📦 بيانات المتجر:', data); // ✅ تأكد إنها توصل
...
// ─── عرض المنتجات (نسخة مبسطة بدون D3.js) ───
function renderItems(items) {
  const grid = document.getElementById('shop-grid');
  if (!grid) {
    console.error('❌ shop-grid مش موجود في الصفحة!');
    return;
  }
  ...
  console.log('🛒 جاري عرض المنتجات:', items); // ✅ تأكد
  ...
  grid.innerHTML = items.map((item) => {
    ...
    return `
      <div class="shop-item-card" onclick="openBuyModal('${item.id}')">
        ...
        <div class="item-icon" style="font-size:40px;text-align:center;margin:8px 0;">
          ${item.emoji || '🎁'}
        </div>
